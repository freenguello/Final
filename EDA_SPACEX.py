...
color = cm.inferno_r(np.linspace(.1, 1, 12))
color
import random
x = [{i: random.randint(1, 12)} for i in range(12)]
df.groupby(['Orbit']).mean()['Class'].plot(kind='bar' , color=color, legend=False, figsize=(12, 4))
...
sns.catplot(x='PayloadMass',y='Outcome',data=df,hue='Class', aspect = 3)
